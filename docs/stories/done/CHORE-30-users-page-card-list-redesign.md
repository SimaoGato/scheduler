...
- **Pre-existing bug re-check (Step 1)**: re-confirmed at implementation
  start that `app/[locale]/(app)/admin/users/page.tsx` already destructures
  `{ data, error }` with a `console.error` guard — no drift since the
  grounding note's `git log -p` check, no fix needed, no diff made to this
  file.
- **AC1**: `e2e-integration/users-card-list.spec.ts`'s two AC1 tests assert,
  against the real seeded `ADMIN_ID`/`MEMBER_ID` rows: avatar initials text
  (`CA`/`CM`, derived from `display_name`), display name, email, and role
  badge class/text per role (`bg-header`/`text-header-foreground` for
  admin, `border`/`text-muted-foreground` for member, both asserted absent
  on the wrong role). Both pass.
- **AC2**: the throwaway-fixture promote/demote DOM test
  (`e2e-integration/users-card-list.spec.ts`) clicks `um-promote-<id>` and
  confirms `um-demote-<id>` renders in its place with the badge text
  flipping to "Administrador" — proves the existing testids/handler/API
  call are unchanged and still wired correctly through the new markup.
  `e2e-integration/admin-crud.spec.ts` and `self-demotion.spec.ts` (API-only,
  no DOM dependency) were re-run and remain green, confirming no regression
  to the STORY-08 self-demotion guard or the last-admin 409 guard.
  **E2E_WITH_AUTH manual/local verification gap (same as CHORE-29's own
  precedent)**: genuine Google-OAuth-gated real-browser login is not
  available in this sandbox (no real Google account credentials), so
  `E2E_WITH_AUTH=1 npm run test:e2e` for `e2e/user-management.spec.ts`'s
  STORY-08 AC1-AC3 tests and `e2e/user-table-alignment.spec.ts`'s AC2 test
  could not be executed end-to-end with a real session. As a substitute:
  (1) mechanical diff verification — `git diff` on both files shows the
  `user-management.spec.ts` change is a pure one-line selector swap
  (`tbody tr` → `[data-testid="um-list"] li`) with zero assertion change,
  and the `user-table-alignment.spec.ts` change swaps the `<td>`-based
  geometry check for the row-relative one specified in Design decision 3,
  keeping the same numeric tolerance/padding constants; (2) the new
  CI-enforced `e2e-integration/users-card-list.spec.ts` exercises the same
  underlying behavior (promote/demote click, tap-target height, `um-list`
  scoping) end-to-end against a real authenticated admin session via the
  `adminPage` fixture (real `signInWithPassword` cookies against local
  Supabase), which is the closest available substitute for the gated
  suite's own coverage.
- **AC3**: `e2e-integration/users-card-list.spec.ts` seeds a throwaway user
  with a deliberately long pt-PT display name and a long email, then at
  both 375px and 390px asserts (a) `document.documentElement.scrollWidth`
  does not exceed the viewport width, and (b) the email `<p className=
  "truncate">` node's `scrollWidth` exceeds its `clientWidth` (proving
  `truncate` actually engages, not merely "happened to fit"). Screenshots
  were captured at both widths
  (`test-results-integration/chore-30-users-card-list-{375,390}.png`,
  gitignored per `.gitignore`'s `/test-results-integration/` rule, same as
  CHORE-29's own screenshot artifacts — not committed to git, but generated
  locally and visually reviewed during implementation, and regenerated by
  CI as job artifacts on every run). Visual review confirms: avatar +
  truncated name/email stay left-grouped, role badge + action button stay
  right-grouped and flush, no incoherent wrapping at either width — matches
  the BUGFIX-06 coherence bar, not just a scrollWidth pass.
- **AC4**: `e2e-integration/users-card-list.spec.ts`'s dedicated tap-target
  test asserts the promote/demote button's `boundingBox()` height is
  `>= 44` at 375px, after a `toBeVisible()` wait (CLAUDE.md's
  `boundingBox()` guard convention).
- **AC5**: `['header', 'header-foreground']` added to
  `EXISTING_SEMANTIC_PAIRS` in `e2e/design-language-foundation.spec.ts`;
  the full smoke suite (`npm run test:e2e`) confirms this pair still meets
  WCAG AA (16.38:1 light / 16.12:1 dark, cited from CHORE-28, no
  re-measurement needed since no token value changed). `primary`/
  `primary-foreground` (avatar) and `card`/`card-foreground` (row surface)
  were already gated; `muted-foreground`/`card` (member badge text on the
  row surface) was already gated by `e2e/card-ui-primitive.spec.ts` AC2.
  No new token values were introduced anywhere in this story.
- **Visual QA (Technical notes' "both themes at 375px and 1280px")**:
  performed via a throwaway Playwright script (not committed) built on
  `e2e-integration/fixtures.ts`'s `adminPage` fixture, toggling dark mode
  via the real `/settings` theme-toggle control
  (`theme-toggle-dark`, per `components/ThemeToggle.tsx`) before navigating
  to `/pt-PT/admin/users`. Confirmed in both themes at both widths: the
  admin badge's navy `bg-header` fill stays legible with clear
  `text-header-foreground` contrast (the fill barely changes between
  themes, by design — `--header` is a near-fixed navy surface in both
  `:root` and `.dark`), the member badge's outline pill reads clearly
  against `bg-card`, and the avatar's `bg-primary`/`text-primary-foreground`
  fill inverts correctly between themes with no contrast regression.
- **AC6**: `npm run lint`, `npx tsc --noEmit`, `npm run build`, and
  `npm run test:e2e` all exit 0 (147 passed, 91 skipped — all skips are
  pre-existing `E2E_WITH_AUTH` gates, none newly introduced). The new
  `e2e-integration/users-card-list.spec.ts` and the full
  `npm run test:integration` suite (112 passed, 0 failed) were also run
  locally against a real local Supabase instance and are green, with no
  regressions to any other integration spec (roles-card-list, admin-crud,
  self-demotion, availability, home, mobile-bottom-nav, etc.).
