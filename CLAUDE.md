...
- **`CREATE POLICY IF NOT EXISTS` is PG17+ only**: Supabase runs PostgreSQL 15. For idempotent migrations, use `DROP POLICY IF EXISTS "name" ON table; CREATE POLICY ...` instead (this pattern works on PG15 and later).
