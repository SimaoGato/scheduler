...
- **Native `<details>`/`<summary>` as a lightweight dropdown**: When shadcn `DropdownMenu` is not installed and no React state is required (e.g., a simple show/hide panel), native HTML `<details>`/`<summary>` works in an async Server Component without `'use client'`. The browser handles open/close natively; `<summary>` receives `cursor-pointer` via CSS. Acceptable limitation: clicking outside does NOT close the panel (browsers provide no native outside-click dismissal). Only use this pattern when ACs don't require click-outside behavior.
